 ...
 * See the Java Map Introduction for help.
 */
public class Map_1 {
    /** mapBully
    ...
    /** mapAB3
     * Modify and return the given map as follows: if exactly one of the keys "a" or "b" has a value in the map (but not
     * both), set the other to have that same value in the map.
     */
    ...
    public Map<String, String> mapAB4(Map<String, String> map) {
        if (map.containsKey("a") && map.containsKey("b")) {
            if (map.get("a").length() == map.get("b").length()) {
                ...
                String longer = map.get("a").length() > map.get("b").length()
                        ? map.get("a") : map.get("b");
                map.put("c", longer);
            }
        }
