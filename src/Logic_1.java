...
public class Logic_1 {
    ...
    public String alarmClock(int day, boolean vacation) {
        ...
        if (weekday && vacation || !weekday && !vacation) return ten;
        else if (!weekday && vacation) return off;
        else return seven;
    }
    ...
    public String fizzString(String str) {
        boolean startsf = str.substring(0, 1).equals("f");
        boolean endsb = str.substring(str.length()-1).equals("b");
        if (startsf && endsb) return "FizzBuzz";
        else if (startsf) return "Fizz";
        else if (endsb) return "Buzz";
        else return str;
    }
    ...
    public boolean lessBy10(int a, int b, int c) {
        int greatestDiff = Math.abs(a-b);
        if (Math.abs(a-c) > greatestDiff) greatestDiff = Math.abs(a-c);
        if (Math.abs(b-c) > greatestDiff) greatestDiff = Math.abs(b-c);
        return greatestDiff >= 10;
    }
    ...
    public int sumLimit(int a, int b) {
        ...
        int sum_len = String.valueOf(sum).length();
        if (a_len == sum_len) return sum;
        else return a;
    }
}